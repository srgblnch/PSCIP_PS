...
#include <asm/spinlock.h>			/* Defines spinlock interface. 
									...
									*/
#include <asm/semaphore.h>			/* Defines semaphore interface	*/
...
//interrupt functions
	...
	/** Interrupt hanlder. */
	irqreturn_t pscip_irq_handler(int pciirq, void *dev_id, struct pt_regs *regs);
 ...
 int pscip_assign_irq(pscip_Dev dev)
{
	...
	err = request_irq(
				...
				SA_SHIRQ,																						// PCI interrupts have to be shared
                "pscip",
                dev);
	if (err)
